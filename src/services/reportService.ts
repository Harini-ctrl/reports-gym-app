...
import { ReportFilters, CoachPerformance } from '../config/types';
  ...
  async generateCoachPerformanceReport(filters: ReportFilters): Promise<CoachPerformance[]> {
    ...
    const report = await Promise.all(
      coaches.map(async (coach) => {
        ...
        const email = coach.email || `${this.capitalizeFirstLetter(coach.firstName)}_${this.capitalizeFirstLetter(coach.lastName)}@xyz.com`;
    ...
    // Filter out coaches with zero workouts or zero average feedback
    const filteredReport = report.filter(coach => 
      coach.noOfWorkouts > 0 && coach.averageFeedback > 0
    );
